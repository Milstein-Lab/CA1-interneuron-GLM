...
activity_dict, factors_dict = ut.preprocess_data(filepath, normalize=False)

min_max_activity = {}
for animal in activity_dict:
    animal_min_max = {}
    for neuron in activity_dict[animal]:
        activity = activity_dict[animal][neuron]
        normalized = (activity - np.min(activity)) / (np.max(activity) - np.min(activity))
        animal_min_max[neuron] = normalized
    min_max_activity[animal] = animal_min_max
filtered_factors_dict = ut.subset_variables_from_data(factors_dict, variables_to_keep=["Velocity"])
GLM_params, predicted_activity_dict = ut.fit_GLM_population(filtered_factors_dict, min_max_activity, quintile=None, regression='linear')
residual_activity_dict = ut.get_residual_activity_dict(min_max_activity, predicted_activity_dict)
...
if __name__ == "__main__":
    ...
    cell_of_interestnumpy = cell_of_interest.squeeze(1).numpy()
    reconstruction_TCA = model.construct().squeeze(1).numpy(force=True)

    MSE = np.mean(np.square(cell_of_interestnumpy - reconstruction_TCA))


    save_dir = r"/ocean/projects/bio240068p/mfinch/CA1-inter/data/MSE_SST_positive_x00_min_max"
    os.makedirs(save_dir, exist_ok=True)  # Ensure directory exists

    save_path = os.path.join(save_dir, f"MSE_{ranks}_animal{animal_id}_cell_id{cell_id}.pkl")
