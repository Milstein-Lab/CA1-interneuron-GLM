...
def compute_features(params, network_seed, previous_features=None, model_id=None, export=False, plot=False):
    ...
    include_inh = str_true_false_to_bool(context.include_inh)
    ...
    model.SST_sf_opt = float(SST_sf_opt)
    model.NDNF_sf_opt = float(NDNF_sf_opt)
...
def filter_features(features_dict_list, previous_features, model_id=None, export=False, plot=False):
    ...
    if plot:
        ...
        save_path = "/Users/michaelfinch/CA1-interneuron-GLM/Clean_notebooks_to_date/pkl_exp.pkl"
        with open(save_path, 'wb') as f:
            pickle.dump(important_dict, f)

        plot_multidendrite_EC_err_across_seeds(warped_list_dict[0], residual_activity_dict_EC, tau_ms = tau_ms,
