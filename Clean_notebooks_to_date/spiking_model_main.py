...
class SpikingModel:
    ...
    def prepare_inputs(self, animal_by_animal: bool = False) -> None:
        ...
        if self.cfg.which_velocity == "EC_animal_average":
            ...
            an_velocity = get_velocity_array(factors_dict_EC, factors_dict_SST, fixed_NDNF, which_type="repeated_waveform")
        ...
        plateau_dict_animal, counts_dict_animal = get_plateau_and_cumulative_ragged(padded_warped_activity_dict, dend_threshold, plateau_len=300, refractory=800, scan_step=100)
    ...
    def simulate_seeds(self, animal_by_animal: bool = False, seed_override: int | None = None) -> None:
        ...
        if animal_by_animal:
            ...
            for animal, pwa_cell_dict in pwa_by_animal.items():
                ...
                for i in seeds:
                    epsp_cells, kernel = get_epsp_dict_animal(pwa_cell_dict, tau_ms=tau, amp=1., seed=i)
    ...
    def plot_animal_by_animal(self) -> None:
        ...
        residual_all  = self.data.get("residual_activity_dict_EC", {})
        ...
        for animal, seed_vm_dict in vm_by_animal.items():
            vel = vel_by_animal.get(animal)
            if vel is None:
                print(f"[plot_animal_by_animal] Skipping {animal}: no velocity.")
                continue

            resid = residual_all.get(animal, None)
            print(f"len(resid) {len(resid)}")

            try:
                plot_dendrite_spikes_multiple_seeds(
                    seed_vm_dict,
                    vel,
                    residual_activity_dict_EC=resid,  # per-animal slice
                    animal=animal,
                ...
                # util may not accept residual kwarg
                plot_dendrite_spikes_multiple_seeds(
                    seed_vm_dict,
                    vel,
                    animal=animal,
            ...
            for animal, seed_vm_dict in vm_by_animal.items():
                ...
                try:
                    plot_dendrite_spikes_multiple_seeds(
                        seed_vm_dict,
                        vel,
                        residual_activity_dict_EC=residual[animal],
                        animal=animal,
                        animal_by_animal=True,
                        dend_threshold=self.cfg.dend_threshold,
                        tau=self.cfg.tau_ms,
    ...
    #         try:
    #             plot_dendrite_spikes_multiple_seeds(seed_vm_dict, animal_vel, residual_activity_dict_EC, animal, dend_threshold=self.cfg.dend_threshold, tau=self.cfg.tau_ms, num_seeds=self.cfg.num_seeds)
...
def main(do_plot: bool, save_path: str | None, num_seeds: int, which_velocity: str, animal_by_animal: bool, load_and_plot: str | None, plot_mode: str, grid_index:int, max_seed:int, only_animal:str, single_seed:int, print_animals:bool, tau_ms: float, dend_threshold: float):
...
##### cfg is a configuration object -- holdds paths, tau, seeds, threshold,




############################ animal by animal call ------  
"""
python spiking_model_main.py \
  --animal-by-animal \
  --only-animal animal_3 \
  --single-seed 7 \
  --num-seeds 20 \
  --which-velocity EC_animal_average \
  --no-plot \
  --save-path datasets/run_animal3_seed7.pkl
"""

#### testing plotting 

"""python -u spiking_model_main.py \
  --animal-by-animal \
  --load-and-plot /Users/michaelfinch/CA1-interneuron-GLM/datasets/run_animal_4_tau5.pkl \
  --plot-mode full"""


########################## pooled call 

"""Saved model to: /Users/michaelfinch/CA1-interneuron-GLM/datasets/run_animal3_seed7.pkl
(ca1_env) michaelfinch@cinj3w36zw3 Clean_notebooks_to_date % python spiking_model_main.py \
  --regular \         
  --single-seed 7 \
  --num-seeds 20 \
  --which-velocity EC_animal_average \
  --no-plot \
  --save-path datasets/run_pooled_seed7.pkl """
...
####################################### this will run everything 
    ...
    # an_velocity = get_velocity_array(factors_dict_EC, factors_dict_SST, fixed_filtered_factors_dict_NDNF_newest, which_type="EC_animal_average") #repeated_waveform # EC_animal_average #constant
    ...
    #     epsp_dict, kernel = get_epsp_dict(padded_warped_activity_dict, tau_ms=tau, amp=1., seed=i)

    #     dend_Vm, epsp_list, spike_list = get_dend_vm(epsp_dict, Vrest=-70, epsp_sf = 0.1)
    #     dend_Vm_dict[i] = dend_Vm

    #     print(f"dend_vm ok for seed {i}")

    # just_plateau_starts_sums_dict, plateau_array_dict = get_plateau_array_dict(dend_Vm_dict, dend_threshold)

    # plot_dendrite_spikes_multiple_seeds(dend_Vm_dict, an_velocity, residual_activity_dict_EC, padded_warped_activity_EC, summed_dendrite, just_plateau_starts_sums_dict, plateau_array_dict, dend_threshold=dend_threshold, tau=tau, num_seeds=num_seeds)
################################################
