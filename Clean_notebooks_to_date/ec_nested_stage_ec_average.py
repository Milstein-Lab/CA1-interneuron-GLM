...
from multidendrite_spiking_utils import (get_velocity_array_every_animal, get_scaled_data_Hz_dict, do_the_interpolation_an, get_epsp_dict_animal, get_dend_vm_from_cells_multi, sample_weights,
    get_dendrite_activity_multi, activity_to_dend_vm, get_activity_multidendrite2,_eval_one_seed, _ten_bin_fraction_from_counter, _build_static_inputs, _build_static_inputs_animal_av)
import optuna
...
def config_worker():
    ...
    print(f"[INIT] rank={rank} pid={os.getpid()} RSS={rss_mb:.1f} MB "
        f"dist={context.static['dist']} "
        f"seeds={getattr(context,'seed_range',None)}", flush=True)
...
def compute_features(params, network_seed, model_id=None, export=False, plot=False):
    ...
    # start_pos_cnt50, num_per_dend = _eval_one_seed(paramsdict, int(network_seed), context.static, context.ec_animal_average)
