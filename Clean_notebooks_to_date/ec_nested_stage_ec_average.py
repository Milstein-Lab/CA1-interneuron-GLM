...
def config_worker():
    ...
    print(f"[INIT] spike_trains size: {sizeof_pwa_bytes(static['spike_trains'])/1e6:.1f} MB", flush=True)

    # Print size BEFORE freeing:
    pwa_mb = sizeof_pwa_bytes(context.static["pwa_cell_dict"]) / (1024**2)
    print(f"[INIT] approx pwa_cell_dict size: {pwa_mb:.1f} MB", flush=True)
