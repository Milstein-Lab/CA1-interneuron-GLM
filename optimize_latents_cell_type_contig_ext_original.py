...
def get_per_cell_sliceTCA_reconstruction_contig_original(model, residual_activity_dict_SST, max_clusters=12, animal_id=1, cell_id=2, display=True):
    ...
    for clusters_chosen in range(max_clusters):
        ...
        algo = rpt.Binseg(model="l2", min_size=2).fit(X)
...
if __name__ == "__main__":
    ...
    internals_dict = get_per_cell_sliceTCA_reconstruction_contig_original(model, residual_activity_dict, max_clusters=10, animal_id=animal_id, cell_id=cell_id, display=False)

    save_dir = r"/scratch/msf157/data/CA1-inter/original_contiguouskmeans_EC_reconstruct_MSE_data"
